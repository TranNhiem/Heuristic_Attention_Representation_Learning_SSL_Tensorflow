...
from HARL.DataAugmentations.byol_simclr_multi_croping_augmentation import simclr_augment_randcrop_global_views, simclr_augment_inception_style, \
...
class imagenet_dataset_single_machine():
    ...
    def __init__(self, img_size, train_batch, val_batch, strategy, train_path=None, train_label=None, val_path=None, val_label=None, bi_mask=False,
        ...
        for key in list(FLAGS.Encoder_block_strides.keys()):
            self.feature_size = self.feature_size/ FLAGS.Encoder_block_strides[key]
    ...
    def get_base_train_dataset(self, mask=False):
        ...
        if mask:
            def preprocessing(data):
                return tf.image.resize(tf.image.convert_image_dtype(data["image"], tf.float32), (self.IMG_SIZE, self.IMG_SIZE)), tf.one_hot(data["label"], 1000)
            ds = ( dataset.shuffle(self.BATCH_SIZE * 100, seed=self.seed)
            ...
            def preprocessing(data):
                return tf.image.resize(tf.image.convert_image_dtype(data["image"], tf.float32),
                                       (self.IMG_SIZE, self.IMG_SIZE)), tf.one_hot(data["label"], 1000)
    ...
    def simclr_inception_style_crop(self):
        ...
        train_ds = ds.map(lambda x, y: ((simclr_augment_inception_style(x, self.IMG_SIZE), y),(simclr_augment_inception_style(x, self.IMG_SIZE), y))
                              , num_parallel_calls=AUTO)\
            .batch(self.BATCH_SIZE, num_parallel_calls=AUTO)\
            .prefetch(20)  # .prefetch(AUTO)
    ...
    def simclr_random_global_crop(self):
        ...
        train_ds = ds.map(lambda x, y: ((simclr_augment_randcrop_global_views(x, self.IMG_SIZE), y),(simclr_augment_randcrop_global_views(x, self.IMG_SIZE), y))
                              , num_parallel_calls=AUTO)\
            .batch(self.BATCH_SIZE, num_parallel_calls=AUTO)\
            .prefetch(20)  # .prefetch(AUTO)
        # adding the distribute data to GPUs
