...
class Conv2dFixedPadding(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
    ...
    def __init__(self,
        ...
        if strides > 1:
            self.fixed_padding = FixedPadding(
...
class SK_Conv2D(tf.keras.layers.Layer):  # pylint: disable=invalid-name
    ...
    def call(self, inputs, training):
        ...
        inputs = tf.stack(
            tf.split(inputs, num_or_size_splits=2, axis=channel_axis))
        ...
        global_features = self.batch_norm_relu_1(
            global_features, training=training)
        mixing = self.conv2d_1(global_features, training=training)
        mixing = tf.stack(
            tf.split(mixing, num_or_size_splits=2, axis=channel_axis))
