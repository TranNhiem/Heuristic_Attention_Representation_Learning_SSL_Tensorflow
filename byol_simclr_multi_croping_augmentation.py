...
def rand_distribe_crop_global_local_views_flip(image, crop_size, min_scale, max_scale, high_resol=True):
    ...
    # Get crop_size
    rnd_flp_crp = tf.image.random_crop(
        resz_flp_img, (size, size, image.shape[2]))
...
def rand_distribe_crop_global_local_views_flip_v1(image, crop_size, IMG_SIZE):
    ...
    # w_crop = h_crop * tf.random.uniform(shape=[], minval=0.5, maxval=1.0)

    h_crop = crop_size * 1.4

    w_crop = crop_size * 1.4
    h_crop, w_crop = tf.cast(h_crop, tf.int32), tf.cast(w_crop, tf.int32)

    size = tf.random.uniform(shape=(1,), minval=min_scale*h_crop,
                             maxval=max_scale*h_crop, dtype=tf.float32)
    size = tf.cast(size, tf.int32)[0]
    ...
    if opposite_aspectratio < 0.5:
        h_crop, w_crop = w_crop, h_crop
    image = tf.image.random_crop(image, size=[h_crop, w_crop, image.shape[2]])
    ...
    # Resizing to original size
    image = tf.image.resize(image, size=[crop_size, crop_size])
...
def simclr_augment_randcrop_global_view_image_mask(image, mask_obj, mask_back, IMG_SIZE, feature_size):
    ...
    stacked_image = tf.concat([image, mask_obj, mask_back], axis=2)
    ...
    mask_obj = stacked_image[:, :, 3]
    mask_back = stacked_image[:, :, 4]
    # print(image.shape)
    # print(mask.shape)
    ...
    mask_obj = tf.image.resize(tf.expand_dims(mask_obj, axis=-1), (feature_size, feature_size))
    mask_back = tf.image.resize(tf.expand_dims(mask_back, axis=-1), (feature_size, feature_size))
...
@tf.function
def simclr_augment_randcrop_global_view_image_mask_tf_py(image, mask, IMG_SIZE, z):
